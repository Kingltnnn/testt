...
function UILib:CreateUI()
	...
	Frame.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch and (not isUsingSlider) then
			dragging = true
	...
	Background.Image = "rbxassetid://97133242814989"
	...
	ImageLabel.Image = "rbxassetid://139164748850995"
		...
		function Tab:Section(sectionTitle)
			...
			function Section:DropDown(placeholderTitle, ItemList, callback)
				...
				for i,v in pairs(ItemList) do
					...
					Frame.InputBegan:Connect(function(Input)
						if Input.UserInputType == Enum.UserInputType.Touch and Input.UserInputState == Enum.UserInputState.Begin and (not Debounce) then		
							Debounce = true
							callback()
							game.TweenService:Create(UIStroke, TweenInfo.new(TweenTime), {Color=Color3.fromRGB(0,155,255)}):Play()
							Frame:TweenSize(
